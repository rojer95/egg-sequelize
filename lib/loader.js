...
module.exports = app => {
  ...
  function loadDatabase(config = {}) {
    if (typeof config.ignore === "string" || Array.isArray(config.ignore)) {
      app.deprecate(
        `[egg-sequelize] if you want to exclude ${config.ignore} when load models, please set to config.sequelize.exclude instead of config.sequelize.ignore`
      );
    ...
    if (Array.isArray(config.pluginInject) && config.pluginInject.length > 0) {
      for (const pluginDirname of config.pluginInject) {
        modelDir.push(path.join(pluginDirname, "app", config.baseDir));
      }
    }
  ...
  async function authenticate(database) {
    ...
    try {
      ...
      app.logger.warn(
        `Sequelize Error: ${e.message}, sleep 1 seconds to retry...`
      );
      await sleep(1000);
